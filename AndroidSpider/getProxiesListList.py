...
def getProxies(urllist=[],searchstr=[]):
    '''
    功能：爬取高匿IP构造原始代理IP池
    '''
    init_proxies = []
    if len(urllist) == 0:
        urllist = ["http://www.xicidaili.com/nn/"+str(i) for i in range(1,3)]#西刺高匿IP
    if len(searchstr) == 0:
        searchstr = ['//tr/td[2]/text()','//tr/td[3]/text()','//tr/td[9]/text()','//tr/td[10]/text()']#ip+port+sur_time+ver_time
    for url in urllist:
        ...
        ip_addrs = selector.xpath(searchstr[0])  #IP地址
        port = selector.xpath(searchstr[1])  #端口
        sur_time = selector.xpath(searchstr[2])  #存活时间
        ver_time = selector.xpath(searchstr[3])  #验证时间
...
def testProxy(curr_ip):
    ...
    tarURL = "https://blog.csdn.net/qq_33733970/article/details/80175650"
    user_agent = GetUserAgent()
...
def getProxiesFrom(urllist=[],searchstr=[]):
    init_proxies = getProxies(urllist,searchstr)  #获取原始代理IP
